...
CryptoHelperMac::CryptoHelperMac() {
...
void CryptoHelperMac::generateKeyPair() {
    rsa = RSA_generate_key(kBits, kExp, 0, 0);
}

const string CryptoHelperMac::exportPrivateKey() const {
    if (rsa == NULL) {
        throw logic_error(
...
const string CryptoHelperMac::exportPublicKey() const {
    if (rsa == NULL) {
        throw logic_error(
                string("Export not initialized.Call generateKeyPair first."));
    }
...
const string CryptoHelperMac::signString(const void* privateKey,
...
const string CryptoHelperMac::Opensslb64Encode(size_t slen,
