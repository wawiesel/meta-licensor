 ...
 */
void parseUUID(const char *uuid, unsigned char* buffer_out,
	...
	if (diskInfos == NULL) {
		*disk_info_size = currentDrive;
		...
		if (disk_by_uuid_dir == NULL) {
			LOG_WARN("Open /dev/disk/by-uuid fail",0);
			free(statDrives);
			return FUNC_RET_ERROR;
		}
		result = FUNC_RET_OK;
...
FUNCTION_RETURN verifySignature(const char* stringToVerify,
	...
	BIO_free(bio);
	if (rsa == NULL) {
		LOG_ERROR("Error reading public key",0);
		return FUNC_RET_ERROR;
	}
...
//Can test here if len == decodeLen - if not, then return an error
	...
	if (!(mdctx = EVP_MD_CTX_create())) {
		...
		LOG_ERROR("Error creating context",0);
		return FUNC_RET_ERROR;
	}
	if (1 != EVP_DigestVerifyInit(mdctx, NULL, EVP_sha256(), NULL, pkey)) {
		LOG_ERROR("Error initializing digest",0);
		free_resources(pkey, mdctx);
		return FUNC_RET_ERROR;
	}
